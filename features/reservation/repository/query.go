...
func (r *reservationRepository) CheckAvailability(input reservation.ReservationCore) (data reservation.ReservationCore, err error) {
	...
	tx := r.db.Preload("Homestay").Not("booked_start BETWEEN ? AND ? AND booked_end BETWEEN ? AND ?", input.StartDate, input.EndDate, input.StartDate, input.EndDate).First(&reservation)
	if tx.Error != nil {
